...
class AVLNode:
    ...
    def __init__(self):
        ...
        self.num = 0  # initialize empty node, which has height -1 so num=0
    ...
    def _recalc_height(self):
        ...
        self.num = 1 + self.left.num + self.right.num
...
class AVLDict:
    ...
    def update(self, key, item):
        ...
        node.right = AVLNode()
        node.height = 0
        node.num = 1  # when the node is a leaf(height=0), then num=1

        # need to fix num in the find and fix function
    ...
    def ith_key(self, index: int):
        ...
        """
        # temporary: get list of all the items and return the key so I have
        # an idea of how this works. this runs in O(n) time which is
        # not acceptable for this exercise
        # l=self.items()
        # return l[index][0]

        # need to modify avlnode to have a num property which
        # then can be used to do the key lookup in nlogn time

        # after staring for hours at trees, you eventually see that the index of
        # a given node in the sorted list is the num of the left child of that node

        # after a few more hours, notice that you can't say the same thing for
        # right childs.
        # then notice that for the other nodes, it's parent index + left child.num + 1

        # breathe a sigh of relief that your code does actually work and that you just
        # suck at noticing patterns

        currentnode = self.root
        currentindex = self.root.left.num

        while currentindex != index:
            if index < currentindex:
                # go left
                currentnode = currentnode.left
                currentindex = currentnode.left.num
            elif currentindex < index:
                # go right
                currentnode = currentnode.right
                try:
                    currentindex = currentindex + 1 + currentnode.left.num
                except:
                    pass
        # now, we should have found the node that matches the index we want
        return currentnode.key


def test_ith_key(tree):
    """
    Tests the ith_key function by indexing the list from the items() function
    and comparing the results
    """
    correct = tree.items()
    for i in range(len(correct)):
        if correct[i][0] != tree.ith_key(i):
            print("Error: Key indexed at {} not correct".format(i))


if __name__ == "__main__":
    """
    Entering "python3 avl_dict.py" will run tests on the test_ith_key
    function using two different trees
    """
    print("Running tests...")
