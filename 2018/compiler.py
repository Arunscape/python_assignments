...
def tests():
    ...
    >>> This='Where my tests begin...'

    >>> do_test('max(4,3,45,76,7,5,4,232,2,3,44,56,5,677,2,23,4342,433,234,57,876,4,12,22332,543,3465,456654,657,65,7,3342)')
    (None, ['# code for:', '# max(4,3,45,76,7,5,4,232,2,3,44,56,5,677,2,23,4342,433,234,57,876,4,12,22332,543,3465,456654,657,65,7,3342)', '_result = max(4,3,45,76,7,5,4,232,2,3,44,56,5,677,2,23,4342,433,234,57,876,4,12,22332,543,3465,456654,657,65,7,3342)'])

    >>> do_test('x=1+max(3,2,1,6,8,9)+(y=6)+y+2*y')
    (None, ['# code for:', '# x=1+max(3,2,1,6,8,9)+(y=6)+y+2*y', '_y_1 = 6', '_x_1 = ((((1 + max(3,2,1,6,8,9)) + _y_1) + _y_1) + (2 * _y_1))', '_result = _x_1', '# Update and cleanup x', 'x = _x_1', 'del(_x_1)', '# Update and cleanup y', 'y = _y_1', 'del(_y_1)'])

    >>> do_test('(x=1)+x+ x=2+(x = 1) + (y = x + 1)+(z=y+1)')
    (None, ['# code for:', '# (x=1)+x+ x=2+(x = 1) + (y = x + 1)+(z=y+1)', '_x_1 = 1', '_x_2 = 1', '_y_1 = (_x_2 + 1)', '_z_1 = (_y_1 + 1)', '_x_3 = (((2 + _x_2) + _y_1) + _z_1)', '_result = ((_x_1 + _x_1) + _x_3)', '# Update and cleanup x', 'x = _x_3', 'del(_x_1)', 'del(_x_2)', 'del(_x_3)', '# Update and cleanup y', 'y = _y_1', 'del(_y_1)', '# Update and cleanup z', 'z = _z_1', 'del(_z_1)'])

    >>> do_test('x=1+y=2+z=3+x')
    (None, ['# code for:', '# x=1+y=2+z=3+x', '_z_1 = (3 + x)', '_y_1 = (2 + _z_1)', '_x_1 = (1 + _y_1)', '_result = _x_1', '# Update and cleanup x', 'x = _x_1', 'del(_x_1)', '# Update and cleanup y', 'y = _y_1', 'del(_y_1)', '# Update and cleanup z', 'z = _z_1', 'del(_z_1)'])

    >>> do_test('x=5+ x=x*(x-1)')
    (None, ['# code for:', '# x=5+ x=x*(x-1)', '_x_1 = (x * (x - 1))', '_x_2 = (5 + _x_1)', '_result = _x_2', '# Update and cleanup x', 'x = _x_2', 'del(_x_1)', 'del(_x_2)'])

    >>> do_test('(x=sqr(5))+(y=neg(x))')
    (None, ['# code for:', '# (x=sqr(5))+(y=neg(x))', '_x_1 = (5 ** 2)', '_y_1 = (-_x_1)', '_result = (_x_1 + _y_1)', '# Update and cleanup x', 'x = _x_1', 'del(_x_1)', '# Update and cleanup y', 'y = _y_1', 'del(_y_1)'])

    >>> The= 'test below fails on purpose'
