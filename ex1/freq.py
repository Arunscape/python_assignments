...
def get_words():
    ...
    if(args.ignorecase):
        allwords=[word.lower() for word in allwords] #lowercase all the words!

    if(args.rmpunct):
        #remove all punctuation
        allwords=[''.join(char for char in word if not curses.ascii.ispunct(char) ) for word in allwords]
...
def print_table(wordict,numwords):
    ...
    for word, count in tabledata:
        ...
        print('{0} {1} {2}   {3}'.format(word,initialspace,count,round(count/numwords,2)))
